...
namespace CadastroVacinacao.DAO
{
    class RegistroDao
    {
        ...
        public DataTable GetRegistroById(int id)
        {
            ...
            {
                conexao.AbrirConexao();
                sqlStr = "select reg.id, reg.nome, reg.dataVacinacao, vac.nome, vac.fabricante, vac.origem, vac.dose from registro reg join vacina vac on reg.idVacina = vac.id where reg.id ='" + id + "'"; 
                sql = new MySqlCommand(sqlStr, conexao.con);
                MySqlDataAdapter da = new MySqlDataAdapter();
                da.SelectCommand = sql;
                DataTable dt = new DataTable();
